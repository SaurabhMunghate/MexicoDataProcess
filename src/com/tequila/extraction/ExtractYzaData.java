...
public class ExtractYzaData {
	...
	public static void main(String[] args) throws Exception {
		...
		String rawFilePath="/home/mypremserver/MexicoCache/Cache/yza.mx/yzaData.txt";
		...
		for (String listing : lisintgs) {
			...
			if (uniqueSet.add((companyName+streetAddress+state).toLowerCase())) {
				String out[]= {""+(i++),U.toTitleCase(sicdetails[0]),U.toTitleCase(sicdetails[1]),U.toTitleCase(sicdetails[3]),U.toTitleCase(sicdetails[4]),U.toTitleCase(sicdetails[5]),U.toTitleCase(sicdetails[6]),U.toTitleCase(companyName),U.toTitleCase(streetAddress),U.toTitleCase(neighb),U.toTitleCase(city),U.toTitleCase(state),zip.trim(),null,null,"https://www.yza.mx/",null,null,null,null,null,null,lat,lon,"https://www.yza.mx/Sucursales/Busqueda",U.getTodayDate()};
				writer.writeNext(out);
				U.log(Arrays.toString(out));
			}
//			break;
		}
		FileUtil.writeAllText(U.getCachePath()+"YZA_Branches.csv", sw.toString());
		sw.close();
		writer.close();
	}
